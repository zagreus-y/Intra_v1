"""
Enhanced Portfolio Backtest Engine V2.2 with SignalRanker
...
- SMART SIGNAL RANKING (no longer FIFO)
- Rank competing signals by momentum, volume, volatility

Features:
- Multi-symbol backtesting
- Smart capital allocation with minimums
- Signal ranking by quality (momentum + volume + volatility)
...
try:
    from strategies.signal_ranker import SignalRanker
...
class PortfolioBacktestEngineV2:
    ...
    Ranks competing signals by quality instead of using FIFO.
    """
    ...
    def __init__(
        ...
        use_signal_ranking: bool = True  # Use ranked signals, not FIFO
    ):
        """
        Args:
            ...
            use_signal_ranking: Rank signals by quality instead of FIFO
        """
        ...
        self.use_signal_ranking = use_signal_ranking and RANKING_AVAILABLE
        ...
        # Signal ranking
        self.ranker = SignalRanker(lookback_bars=20) if self.use_signal_ranking else None
    ...
    def run(self, data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        for ts in timeline:
            ...
            # Get signals from strategies and feed price/volume to ranker
            signals = []
            for symbol in data:
                ...
                # Feed to ranker for signal quality scoring
                if self.use_signal_ranking and self.ranker:
                    self.ranker.add_bar(symbol, bar["close"], bar["volume"])
            ...
            # === Execute signals (ranked by quality, not FIFO) ===
            if signals:
                if self.use_signal_ranking and self.ranker:
                    # Rank signals by momentum, volume, volatility
                    ranked_signals = self.ranker.rank_signals(signals)
                    execution_signals = [(sym, sig) for sym, sig, _ in ranked_signals]
                else:
                    # Fallback to FIFO if ranking disabled
                    execution_signals = signals
                
                # Execute in order of quality
                for symbol, signal in execution_signals:
